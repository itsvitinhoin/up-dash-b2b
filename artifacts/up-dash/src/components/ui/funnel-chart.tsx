...
function HRing({
  ...
  const scaleY = useSpring(1, { stiffness: 300 - ringIndex * 60, damping: 24 - ringIndex * 3 });
...
function VRing({
  ...
  const extraScale = 1 + (ringIndex / Math.max(totalRings - 1, 1)) * 0.12;
  const scaleX = useSpring(1, { stiffness: 300 - ringIndex * 60, damping: 24 - ringIndex * 3 });
            ...
            const ringFill =
              isInnermost && renderPattern
                ...
                : isInnermost && gradientStops
                  ? `url(#${gradientId})`
                  : undefined;
            return <VRing color={color} d={r.d} fill={ringFill} hovered={hovered} key={`v-ring-${i}`} opacity={r.opacity} ringIndex={i} totalRings={layers} />;
          })}
        </svg>
      </motion.div>
    </motion.div>
...
function SegmentLabel({
  ...
  return (
    <motion.div
      animate={{ opacity: 1 }}
      className={cn("absolute inset-0 flex", isHorizontal ? cn("flex-col items-center", justifyMap[align]) : cn("flex-row items-center", justifyMap[align]))}
      initial={{ opacity: 0 }}
...
export function FunnelChart({
  ...
  const bandColor = gridCfg.bandColor ?? "hsl(var(--muted) / 0.35)";
