...
- **AI Insight Screens:** `/analytics/insight?screen=dashboard|marketing|customers|products` — four distinct prompt contexts: dashboard (revenue/orders/conversion KPIs), marketing (ROAS/spend/leads), customers (registrations/approval rate/buyers), products (top SKUs, level distribution, at-risk/high-conversion counts). Each returns headline + body + 3 bullets, with heuristic fallback when AI is not configured. Insight is cached 1h per (clientId, dateWindow, screen) key.
