...
- **Schema** (`lib/db/src/schema/`): users, clients, customers, products, sellers, orders/order_items, events, creatives (approvedLeads added). All PKs are nanoid TEXT; all timestamps timestamptz; FKs use cascade or set-null. Customer/product denormalized counters are kept in sync by the seeder.
...
- **Analytics endpoints** (`/analytics/*`): dashboard KPIs + daily series, conversion funnel (monotonic, clamped to 100%), paginated customers with RFM segment counts, product ranking, seller leaderboard, state/city geography breakdowns, and marketing performance (ad spend, ROAS, CPL, CPA, platform breakdown, creative metrics — paid UTM attribution via instagram/facebook/google/tiktok UTM sources). All queries are real Drizzle SQL — no mocks.
- **Marketing page** (`/marketing`): Full-featured paid-channel analytics. 8 animated KPI tiles with sparklines + delta badges (Ad Spend, Attr. Revenue, ROAS, Approval Rate, Total Leads, Approved Leads, CPL, CPA — CPL/CPA delta inverted so green = improvement). AI insight block (headline + body + bullets) shared with `/analytics/insight`, with a Refresh button. Three Recharts charts: (1) Spend vs Leads dual-axis `ComposedChart` (Bar for daily spend, Line for daily leads); (2) ROAS Over Time `AreaChart` with an orange dashed reference line at 2× target; (3) Attributed Revenue `AreaChart`. Platform breakdown (META/GOOGLE/TIKTOK) with spend bars + ROAS/leads/clicks. State breakdown (top 5 Brazilian states by paid leads with spend bars + revenue). Sortable creatives table with platform thumbnail (colored avatar fallback when no imageUrl), platform chip, status badge, and color-coded ROAS (green ≥3×, amber 1.5–3×, red <1.5×). CSV export includes all columns. "Connect your ad accounts" empty state when no paid data exists. Route requires a client selected (`requiresClient: true`). Nav entry "Marketing" added to sidebar (Megaphone icon). Backend: `spendOverTime` daily series from prorated creative spend, `stateBreakdown` from events+customers join.
