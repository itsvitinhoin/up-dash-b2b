...
- **CRM Analytics (Task #29):** Customers page rewritten with 7-metric KPI strip (total registrations, approved, approval rate, buyers, no-purchase, avg days to first purchase, avg days between purchases), 3-tab chart card (registrations timeline area chart / by state horizontal bar / by source donut), new table columns (UTM Source, Opportunity badge, First Purchase date), and clickable rows. New Customer Detail page at `/customers/:customerId` shows header card (avatar, name, email, location, UTM attribution badges, segment/opportunity badges, total spent/orders/member since), Customer Journey funnel (visits→registered→approved→product views→cart adds→purchases), and tabbed activity section (Timeline with event log, Orders table, Products purchased table). Search palette updated to navigate directly to customer detail pages.
