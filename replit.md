...
- **Marketing page** (`/marketing`): Dedicated page with 4 KPI tiles (Ad Spend, ROAS, CPL, CPA), summary row (period paid leads, approved leads, approval rate, attributed revenue), time-series charts (leads/revenue tabs), platform breakdown bars (META/GOOGLE/TIKTOK), and a sortable creatives table. CSV export included. Route requires a client selected (`requiresClient: true`). Nav entry "Marketing" added to sidebar (Megaphone icon).
